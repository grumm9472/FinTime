...
public class HeadlesszombieEntity extends Zombie {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(TimeModEntities.HEADLESSZOMBIE, 20, 3, 30));
	}

	public HeadlesszombieEntity(FMLPlayMessages.SpawnEntity packet, Level world) {
		this(TimeModEntities.HEADLESSZOMBIE, world);
	}

	public HeadlesszombieEntity(EntityType<HeadlesszombieEntity> type, Level world) {
		super(type, world);
		xpReward = 5;
		setNoAi(false);
	}
	...
	public static void init() {
		SpawnPlacements.register(TimeModEntities.HEADLESSZOMBIE, SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
