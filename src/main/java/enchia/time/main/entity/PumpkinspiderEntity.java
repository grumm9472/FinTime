...
public class PumpkinspiderEntity extends Monster {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(TimeModEntities.PUMPKINSPIDER, 20, 4, 4));
	}

	public PumpkinspiderEntity(FMLPlayMessages.SpawnEntity packet, Level world) {
		this(TimeModEntities.PUMPKINSPIDER, world);
	}

	public PumpkinspiderEntity(EntityType<PumpkinspiderEntity> type, Level world) {
		super(type, world);
		xpReward = 6;
		setNoAi(false);
	}
	...
	public static void init() {
		SpawnPlacements.register(TimeModEntities.PUMPKINSPIDER, SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
