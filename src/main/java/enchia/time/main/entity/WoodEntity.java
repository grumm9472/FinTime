...
import enchia.time.main.procedures.WoodEntityIsHurtProcedure;
import enchia.time.main.init.TimeModItems;
import enchia.time.main.init.TimeModEntities;

public class WoodEntity extends Monster {
	private final ServerBossEvent bossInfo = new ServerBossEvent(this.getDisplayName(), ServerBossEvent.BossBarColor.GREEN,
			ServerBossEvent.BossBarOverlay.PROGRESS);

	public WoodEntity(FMLPlayMessages.SpawnEntity packet, Level world) {
		this(TimeModEntities.WOOD, world);
	}

	public WoodEntity(EntityType<WoodEntity> type, Level world) {
		super(type, world);
		xpReward = 200;
		setNoAi(false);
	}
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(TimeModItems.WOOD_STONE));
	}
	...
	public boolean hurt(DamageSource source, float amount) {
		...
		WoodEntityIsHurtProcedure.execute();
