...
import enchia.time.main.procedures.QuiverRightClickedInAirProcedure;
import enchia.time.main.item.inventory.QuiverInventoryCapability;
...
public class QuiverItem extends Item {
	...
	public InteractionResultHolder<ItemStack> use(Level world, Player entity, InteractionHand hand) {
		...
		if (entity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openGui(serverPlayer, new MenuProvider() {
				...
				public Component getDisplayName() {
					return new TextComponent("Quiver");
				}
				...
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					...
					return new QuiverguiMenu(id, inventory, packetBuffer);
				}
			}, buf -> {
		...
		QuiverRightClickedInAirProcedure.execute(world, x, y, z, entity);
		return ar;
	}
