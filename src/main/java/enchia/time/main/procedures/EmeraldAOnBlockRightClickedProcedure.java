...
public class EmeraldAOnBlockRightClickedProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		}.getItemStack(world, new BlockPos((int) x, (int) y, (int) z), 1));
		if (((tool)).isEnchantable() && ((tool)).getCount() == 1 && (emeralds).getItem() == Items.EMERALD) {
			enchant_levels = Math.min(18, ((emeralds)).getCount());
			tool = (EnchantmentHelper.enchantItem(new Random(), (tool), (int) enchant_levels, (true)));
			...
			}
			{
				...
				if (_ent != null) {
					...
					_setstack.setCount(((emeralds)).getCount());
					_ent.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null).ifPresent(capability -> {
						if (capability instanceof IItemHandlerModifiable) {
							((IItemHandlerModifiable) capability).setStackInSlot(_sltid, _setstack);
						}
