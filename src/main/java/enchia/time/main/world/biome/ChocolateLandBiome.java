...
import enchia.time.main.world.features.treedecorators.ChocolateLandTrunkDecorator;
import enchia.time.main.world.features.treedecorators.ChocolateLandLeaveDecorator;
import enchia.time.main.world.features.treedecorators.ChocolateLandFruitDecorator;
import enchia.time.main.init.TimeModBlocks;
import enchia.time.main.TimeMod;

import com.google.common.collect.ImmutableList;

public class ChocolateLandBiome {
	private static final ConfiguredSurfaceBuilder<?> SURFACE_BUILDER = SurfaceBuilder.DEFAULT
			.configured(new SurfaceBuilderBaseConfiguration(TimeModBlocks.CAKE_CHOCOLATE.defaultBlockState(),
					TimeModBlocks.BREADBLOCK.defaultBlockState(), TimeModBlocks.BREADBLOCK.defaultBlockState()));
	...
	public static Biome createBiome() {
		...
		biomeGenerationSettings.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION, register("trees", Feature.TREE
				.configured((new TreeConfiguration.TreeConfigurationBuilder(new SimpleStateProvider(TimeModBlocks.WAFFLE_STICK.defaultBlockState()),
						new StraightTrunkPlacer(7, 2, 0), new SimpleStateProvider(TimeModBlocks.CHOCOLATE_LEAVES.defaultBlockState()),
						new SimpleStateProvider(Blocks.OAK_SAPLING.defaultBlockState()),
						new BlobFoliagePlacer(ConstantInt.of(2), ConstantInt.of(0), 3), new TwoLayersFeatureSize(1, 0, 1)))
								.decorators(ImmutableList.of(ChocolateLandLeaveDecorator.INSTANCE, ChocolateLandTrunkDecorator.INSTANCE,
										ChocolateLandFruitDecorator.INSTANCE))
								.build())
	...
	public static void init() {
		Registry.register(BuiltinRegistries.CONFIGURED_SURFACE_BUILDER, new ResourceLocation(TimeMod.MODID, "chocolate_land"), SURFACE_BUILDER);
	...
	private static ConfiguredFeature<?, ?> register(String name, ConfiguredFeature<?, ?> configuredFeature) {
		CONFIGURED_FEATURES.put(new ResourceLocation(TimeMod.MODID, name + "_chocolate_land"), configuredFeature);
