...
import enchia.time.main.world.features.treedecorators.IceTrunkDecorator;
import enchia.time.main.world.features.treedecorators.IceLeaveDecorator;
import enchia.time.main.world.features.treedecorators.IceFruitDecorator;
import enchia.time.main.TimeMod;

import com.google.common.collect.ImmutableList;

public class IceBiome {
	private static final ConfiguredSurfaceBuilder<?> SURFACE_BUILDER = SurfaceBuilder.DEFAULT.configured(new SurfaceBuilderBaseConfiguration(
			Blocks.BLUE_ICE.defaultBlockState(), Blocks.BLUE_ICE.defaultBlockState(), Blocks.BLUE_ICE.defaultBlockState()));
		...
		biomeGenerationSettings.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION, register("trees", Feature.TREE
				.configured((new TreeConfiguration.TreeConfigurationBuilder(new SimpleStateProvider(Blocks.AIR.defaultBlockState()),
						...
						new BlobFoliagePlacer(ConstantInt.of(2), ConstantInt.of(0), 3), new TwoLayersFeatureSize(1, 0, 1)))
								.decorators(ImmutableList.of(IceLeaveDecorator.INSTANCE, IceTrunkDecorator.INSTANCE, IceFruitDecorator.INSTANCE))
								.build())
	...
	public static void init() {
		Registry.register(BuiltinRegistries.CONFIGURED_SURFACE_BUILDER, new ResourceLocation(TimeMod.MODID, "ice"), SURFACE_BUILDER);
	...
	private static ConfiguredFeature<?, ?> register(String name, ConfiguredFeature<?, ?> configuredFeature) {
		CONFIGURED_FEATURES.put(new ResourceLocation(TimeMod.MODID, name + "_ice"), configuredFeature);
