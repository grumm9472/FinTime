...
import enchia.time.main.world.features.treedecorators.MCY25600BEACHTrunkDecorator;
import enchia.time.main.world.features.treedecorators.MCY25600BEACHLeaveDecorator;
import enchia.time.main.world.features.treedecorators.MCY25600BEACHFruitDecorator;
import enchia.time.main.TimeMod;

import com.google.common.collect.ImmutableList;

public class MCY25600BEACHBiome {
	private static final ConfiguredSurfaceBuilder<?> SURFACE_BUILDER = SurfaceBuilder.DEFAULT.configured(new SurfaceBuilderBaseConfiguration(
			Blocks.WATER.defaultBlockState(), Blocks.WATER.defaultBlockState(), Blocks.WATER.defaultBlockState()));
	...
	public static Biome createBiome() {
		...
		biomeGenerationSettings.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION,
				register("trees", Feature.TREE
						.configured((new TreeConfiguration.TreeConfigurationBuilder(new SimpleStateProvider(Blocks.AIR.defaultBlockState()),
								new StraightTrunkPlacer(7, 2, 0), new SimpleStateProvider(Blocks.AIR.defaultBlockState()),
								new SimpleStateProvider(Blocks.OAK_SAPLING.defaultBlockState()),
								new BlobFoliagePlacer(ConstantInt.of(2), ConstantInt.of(0), 3), new TwoLayersFeatureSize(1, 0, 1)))
										.decorators(ImmutableList.of(MCY25600BEACHLeaveDecorator.INSTANCE, MCY25600BEACHTrunkDecorator.INSTANCE,
												MCY25600BEACHFruitDecorator.INSTANCE))
										.build())
		...
		return new Biome.BiomeBuilder().precipitation(Biome.Precipitation.RAIN).biomeCategory(Biome.BiomeCategory.NONE).depth(-0.1f).scale(0f)
				.temperature(0.5f).downfall(0.5f).specialEffects(effects).mobSpawnSettings(mobSpawnInfo.build())
				.generationSettings(biomeGenerationSettings.build()).build();
	}

	public static void init() {
		Registry.register(BuiltinRegistries.CONFIGURED_SURFACE_BUILDER, new ResourceLocation(TimeMod.MODID, "mcy_25600_beach"), SURFACE_BUILDER);
	...
	private static ConfiguredFeature<?, ?> register(String name, ConfiguredFeature<?, ?> configuredFeature) {
		CONFIGURED_FEATURES.put(new ResourceLocation(TimeMod.MODID, name + "_mcy_25600_beach"), configuredFeature);
