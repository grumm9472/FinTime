...
import enchia.time.main.world.features.treedecorators.Mcy100mountainTrunkDecorator;
import enchia.time.main.world.features.treedecorators.Mcy100mountainLeaveDecorator;
import enchia.time.main.world.features.treedecorators.Mcy100mountainFruitDecorator;
import enchia.time.main.init.TimeModBlocks;
import enchia.time.main.TimeMod;

import com.google.common.collect.ImmutableList;

public class Mcy100mountainBiome {
	...
	public static Biome createBiome() {
				...
				register("trees", Feature.TREE
						.configured((new TreeConfiguration.TreeConfigurationBuilder(new SimpleStateProvider(Blocks.AIR.defaultBlockState()),
								...
								new BlobFoliagePlacer(ConstantInt.of(2), ConstantInt.of(0), 3), new TwoLayersFeatureSize(1, 0, 1)))
										.decorators(ImmutableList.of(Mcy100mountainLeaveDecorator.INSTANCE, Mcy100mountainTrunkDecorator.INSTANCE,
												Mcy100mountainFruitDecorator.INSTANCE))
										.build())
		...
		return new Biome.BiomeBuilder().precipitation(Biome.Precipitation.RAIN).biomeCategory(Biome.BiomeCategory.NONE).depth(0.3f).scale(0.3f)
				.temperature(0.5f).downfall(0.5f).specialEffects(effects).mobSpawnSettings(mobSpawnInfo.build())
				.generationSettings(biomeGenerationSettings.build()).build();
	}

	public static void init() {
		Registry.register(BuiltinRegistries.CONFIGURED_SURFACE_BUILDER, new ResourceLocation(TimeMod.MODID, "mcy_100mountain"), SURFACE_BUILDER);
	...
	private static ConfiguredFeature<?, ?> register(String name, ConfiguredFeature<?, ?> configuredFeature) {
		CONFIGURED_FEATURES.put(new ResourceLocation(TimeMod.MODID, name + "_mcy_100mountain"), configuredFeature);
