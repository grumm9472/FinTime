...
import enchia.time.main.world.features.treedecorators.MudTrunkDecorator;
import enchia.time.main.world.features.treedecorators.MudLeaveDecorator;
import enchia.time.main.world.features.treedecorators.MudFruitDecorator;
import enchia.time.main.init.TimeModBlocks;
import enchia.time.main.TimeMod;

import com.google.common.collect.ImmutableList;

public class MudBiome {
	private static final ConfiguredSurfaceBuilder<?> SURFACE_BUILDER = SurfaceBuilder.DEFAULT.configured(new SurfaceBuilderBaseConfiguration(
			TimeModBlocks.MUD_BLOCK.defaultBlockState(), TimeModBlocks.MUD_BLOCK.defaultBlockState(), TimeModBlocks.MUD_BLOCK.defaultBlockState()));
		...
		biomeGenerationSettings.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION, register("trees", Feature.TREE
				.configured((new TreeConfiguration.TreeConfigurationBuilder(new SimpleStateProvider(Blocks.CAVE_AIR.defaultBlockState()),
						...
						new BlobFoliagePlacer(ConstantInt.of(2), ConstantInt.of(0), 3), new TwoLayersFeatureSize(1, 0, 1)))
								.decorators(ImmutableList.of(MudLeaveDecorator.INSTANCE, MudTrunkDecorator.INSTANCE, MudFruitDecorator.INSTANCE))
								.build())
	...
	public static void init() {
		Registry.register(BuiltinRegistries.CONFIGURED_SURFACE_BUILDER, new ResourceLocation(TimeMod.MODID, "mud"), SURFACE_BUILDER);
	...
	private static ConfiguredFeature<?, ?> register(String name, ConfiguredFeature<?, ?> configuredFeature) {
		CONFIGURED_FEATURES.put(new ResourceLocation(TimeMod.MODID, name + "_mud"), configuredFeature);
