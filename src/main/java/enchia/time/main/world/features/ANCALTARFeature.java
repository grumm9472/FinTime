...
public class ANCALTARFeature extends Feature<NoneFeatureConfiguration> {
	public static final ANCALTARFeature FEATURE = (ANCALTARFeature) new ANCALTARFeature().setRegistryName("time:ancaltar");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public ANCALTARFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "anc_altar"));
		if (template == null)
			return false;
