...
public class Ball10Feature extends Feature<NoneFeatureConfiguration> {
	public static final Ball10Feature FEATURE = (Ball10Feature) new Ball10Feature().setRegistryName("time:ball_10");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Ball10Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "409600ball8small3"));
		if (template == null)
			return false;
