...
public class Ball11Feature extends Feature<NoneFeatureConfiguration> {
	public static final Ball11Feature FEATURE = (Ball11Feature) new Ball11Feature().setRegistryName("time:ball_11");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Ball11Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "409600ballhuge6ghoul1"));
		if (template == null)
			return false;
