...
public class Ball12Feature extends Feature<NoneFeatureConfiguration> {
	public static final Ball12Feature FEATURE = (Ball12Feature) new Ball12Feature().setRegistryName("time:ball_12");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Ball12Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "409600ballhuge6ghoul2"));
		if (template == null)
			return false;
