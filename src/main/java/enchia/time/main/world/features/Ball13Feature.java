...
public class Ball13Feature extends Feature<NoneFeatureConfiguration> {
	public static final Ball13Feature FEATURE = (Ball13Feature) new Ball13Feature().setRegistryName("time:ball_13");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Ball13Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "409600ballhuge6ghoul3"));
		if (template == null)
			return false;
