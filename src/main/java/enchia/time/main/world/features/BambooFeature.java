...
public class BambooFeature extends Feature<NoneFeatureConfiguration> {
	public static final BambooFeature FEATURE = (BambooFeature) new BambooFeature().setRegistryName("time:bamboo");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = Set.of(new ResourceLocation("time:mcy_25600_mabboo"));
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "bambootree"));
		if (template == null)
			return false;
