...
public class BluebigFeature extends Feature<NoneFeatureConfiguration> {
	public static final BluebigFeature FEATURE = (BluebigFeature) new BluebigFeature().setRegistryName("time:bluebig");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public BluebigFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "bluebigmcy1600"));
		if (template == null)
			return false;
