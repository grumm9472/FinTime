...
public class CandyStickStr10Feature extends Feature<NoneFeatureConfiguration> {
	public static final CandyStickStr10Feature FEATURE = (CandyStickStr10Feature) new CandyStickStr10Feature()
			.setRegistryName("time:candy_stick_str_10");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public CandyStickStr10Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "candystickbig_4"));
		if (template == null)
			return false;
