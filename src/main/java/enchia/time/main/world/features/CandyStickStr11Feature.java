...
public class CandyStickStr11Feature extends Feature<NoneFeatureConfiguration> {
	public static final CandyStickStr11Feature FEATURE = (CandyStickStr11Feature) new CandyStickStr11Feature()
			.setRegistryName("time:candy_stick_str_11");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public CandyStickStr11Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "candystickbig_3"));
		if (template == null)
			return false;
