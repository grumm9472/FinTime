...
public class CandyStickStr12Feature extends Feature<NoneFeatureConfiguration> {
	public static final CandyStickStr12Feature FEATURE = (CandyStickStr12Feature) new CandyStickStr12Feature()
			.setRegistryName("time:candy_stick_str_12");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public CandyStickStr12Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "back_stick"));
		if (template == null)
			return false;
