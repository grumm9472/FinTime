...
public class CandyStickStr1Feature extends Feature<NoneFeatureConfiguration> {
	public static final CandyStickStr1Feature FEATURE = (CandyStickStr1Feature) new CandyStickStr1Feature().setRegistryName("time:candy_stick_str_1");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public CandyStickStr1Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "10"));
		if (template == null)
			return false;
