...
public class CandyStickStr3Feature extends Feature<NoneFeatureConfiguration> {
	public static final CandyStickStr3Feature FEATURE = (CandyStickStr3Feature) new CandyStickStr3Feature().setRegistryName("time:candy_stick_str_3");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public CandyStickStr3Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "8"));
		if (template == null)
			return false;
