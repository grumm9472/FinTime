...
public class CandyStickStr4Feature extends Feature<NoneFeatureConfiguration> {
	public static final CandyStickStr4Feature FEATURE = (CandyStickStr4Feature) new CandyStickStr4Feature().setRegistryName("time:candy_stick_str_4");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public CandyStickStr4Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "9"));
		if (template == null)
			return false;
