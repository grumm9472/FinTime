...
public class CandyStickStr5Feature extends Feature<NoneFeatureConfiguration> {
	public static final CandyStickStr5Feature FEATURE = (CandyStickStr5Feature) new CandyStickStr5Feature().setRegistryName("time:candy_stick_str_5");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public CandyStickStr5Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "andystickbig_5"));
		if (template == null)
			return false;
