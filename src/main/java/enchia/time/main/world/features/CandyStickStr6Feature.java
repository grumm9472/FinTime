...
public class CandyStickStr6Feature extends Feature<NoneFeatureConfiguration> {
	public static final CandyStickStr6Feature FEATURE = (CandyStickStr6Feature) new CandyStickStr6Feature().setRegistryName("time:candy_stick_str_6");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public CandyStickStr6Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "andystickbig_6"));
		if (template == null)
			return false;
