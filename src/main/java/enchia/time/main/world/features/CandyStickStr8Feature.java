...
public class CandyStickStr8Feature extends Feature<NoneFeatureConfiguration> {
	public static final CandyStickStr8Feature FEATURE = (CandyStickStr8Feature) new CandyStickStr8Feature().setRegistryName("time:candy_stick_str_8");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public CandyStickStr8Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "candystickbig_1"));
		if (template == null)
			return false;
