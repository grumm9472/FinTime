...
public class CandyStickStr9Feature extends Feature<NoneFeatureConfiguration> {
	public static final CandyStickStr9Feature FEATURE = (CandyStickStr9Feature) new CandyStickStr9Feature().setRegistryName("time:candy_stick_str_9");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public CandyStickStr9Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "candystickbig_2"));
		if (template == null)
			return false;
