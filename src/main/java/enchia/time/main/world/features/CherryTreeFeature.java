...
public class CherryTreeFeature extends Feature<NoneFeatureConfiguration> {
	public static final CherryTreeFeature FEATURE = (CherryTreeFeature) new CherryTreeFeature().setRegistryName("time:cherry_tree");
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "cherrytree"));
		if (template == null)
			return false;
