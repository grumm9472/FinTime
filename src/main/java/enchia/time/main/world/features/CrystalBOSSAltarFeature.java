...
public class CrystalBOSSAltarFeature extends Feature<NoneFeatureConfiguration> {
	public static final CrystalBOSSAltarFeature FEATURE = (CrystalBOSSAltarFeature) new CrystalBOSSAltarFeature()
			.setRegistryName("time:crystal_boss_altar");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public CrystalBOSSAltarFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "crystalbossaltar"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 1000) {
			...
			for (int a = 0; a < count; a++) {
				...
				BlockPos spawnTo = new BlockPos(i + 0, j + 100, k + 0);
