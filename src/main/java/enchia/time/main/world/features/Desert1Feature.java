...
public class Desert1Feature extends Feature<NoneFeatureConfiguration> {
	public static final Desert1Feature FEATURE = (Desert1Feature) new Desert1Feature().setRegistryName("time:desert_1");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Desert1Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "desert5x5_cactusbig"));
		if (template == null)
			return false;
