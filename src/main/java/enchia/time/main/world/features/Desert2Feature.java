...
public class Desert2Feature extends Feature<NoneFeatureConfiguration> {
	public static final Desert2Feature FEATURE = (Desert2Feature) new Desert2Feature().setRegistryName("time:desert_2");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Desert2Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "desert5x5_cactusbig2"));
		if (template == null)
			return false;
