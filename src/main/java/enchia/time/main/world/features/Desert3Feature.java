...
public class Desert3Feature extends Feature<NoneFeatureConfiguration> {
	public static final Desert3Feature FEATURE = (Desert3Feature) new Desert3Feature().setRegistryName("time:desert_3");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Desert3Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "desert5x5_cactusbig3"));
		if (template == null)
			return false;
