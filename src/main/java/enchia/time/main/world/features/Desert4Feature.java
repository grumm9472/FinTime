...
public class Desert4Feature extends Feature<NoneFeatureConfiguration> {
	public static final Desert4Feature FEATURE = (Desert4Feature) new Desert4Feature().setRegistryName("time:desert_4");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Desert4Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "desert5x9_smalldeadtree"));
		if (template == null)
			return false;
