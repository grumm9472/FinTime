...
public class Desert5Feature extends Feature<NoneFeatureConfiguration> {
	public static final Desert5Feature FEATURE = (Desert5Feature) new Desert5Feature().setRegistryName("time:desert_5");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Desert5Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "desert5x9_smalldeadtree2"));
		if (template == null)
			return false;
