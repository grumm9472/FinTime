...
public class Desert7Feature extends Feature<NoneFeatureConfiguration> {
	public static final Desert7Feature FEATURE = (Desert7Feature) new Desert7Feature().setRegistryName("time:desert_7");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Desert7Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "desert-1x-1_sandrock2"));
		if (template == null)
			return false;
