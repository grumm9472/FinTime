...
public class Desert8Feature extends Feature<NoneFeatureConfiguration> {
	public static final Desert8Feature FEATURE = (Desert8Feature) new Desert8Feature().setRegistryName("time:desert_8");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Desert8Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "desert1x1_sandrock1"));
		if (template == null)
			return false;
