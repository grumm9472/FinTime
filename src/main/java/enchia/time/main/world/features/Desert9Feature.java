...
public class Desert9Feature extends Feature<NoneFeatureConfiguration> {
	public static final Desert9Feature FEATURE = (Desert9Feature) new Desert9Feature().setRegistryName("time:desert_9");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Desert9Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "desert1x1_sandrock2"));
		if (template == null)
			return false;
