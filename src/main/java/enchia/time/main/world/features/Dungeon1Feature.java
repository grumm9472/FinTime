...
public class Dungeon1Feature extends Feature<NoneFeatureConfiguration> {
	public static final Dungeon1Feature FEATURE = (Dungeon1Feature) new Dungeon1Feature().setRegistryName("time:dungeon_1");
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "dungeon1"));
		if (template == null)
			return false;
