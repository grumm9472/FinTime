...
public class Foresttree3Feature extends Feature<NoneFeatureConfiguration> {
	public static final Foresttree3Feature FEATURE = (Foresttree3Feature) new Foresttree3Feature().setRegistryName("time:foresttree_3");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Foresttree3Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "cloudtreemcy100"));
		if (template == null)
			return false;
