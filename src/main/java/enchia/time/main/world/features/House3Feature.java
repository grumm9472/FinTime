...
public class House3Feature extends Feature<NoneFeatureConfiguration> {
	public static final House3Feature FEATURE = (House3Feature) new House3Feature().setRegistryName("time:house_3");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public House3Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "102400_house_3"));
		if (template == null)
			return false;
