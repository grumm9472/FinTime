...
public class House5Feature extends Feature<NoneFeatureConfiguration> {
	public static final House5Feature FEATURE = (House5Feature) new House5Feature().setRegistryName("time:house_5");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public House5Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "102400_house_5"));
		if (template == null)
			return false;
