...
public class IceDungeonFeature extends Feature<NoneFeatureConfiguration> {
	public static final IceDungeonFeature FEATURE = (IceDungeonFeature) new IceDungeonFeature().setRegistryName("time:ice_dungeon");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public IceDungeonFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "icedungeon"));
		if (template == null)
			return false;
		if ((context.random().nextInt(1000000) + 1) <= 90000) {
