...
public class IceTreeNew2Feature extends Feature<NoneFeatureConfiguration> {
	public static final IceTreeNew2Feature FEATURE = (IceTreeNew2Feature) new IceTreeNew2Feature().setRegistryName("time:ice_tree_new_2");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private final List<Block> base_blocks = List.of(TimeModBlocks.MCY_400GRASS, TimeModBlocks.FORZEN_GRASS);
	private StructureTemplate template = null;

	public IceTreeNew2Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "icetreesmall"));
		if (template == null)
			return false;
