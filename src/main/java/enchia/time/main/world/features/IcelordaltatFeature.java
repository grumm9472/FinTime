...
public class IcelordaltatFeature extends Feature<NoneFeatureConfiguration> {
	public static final IcelordaltatFeature FEATURE = (IcelordaltatFeature) new IcelordaltatFeature().setRegistryName("time:icelordaltat");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public IcelordaltatFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "xplordaltar"));
		if (template == null)
			return false;
