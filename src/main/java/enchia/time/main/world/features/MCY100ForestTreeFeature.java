...
public class MCY100ForestTreeFeature extends Feature<NoneFeatureConfiguration> {
	public static final MCY100ForestTreeFeature FEATURE = (MCY100ForestTreeFeature) new MCY100ForestTreeFeature()
			.setRegistryName("time:mcy_100_forest_tree");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public MCY100ForestTreeFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "bigmcy100foresttree"));
		if (template == null)
			return false;
