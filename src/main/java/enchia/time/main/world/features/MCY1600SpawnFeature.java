...
public class MCY1600SpawnFeature extends Feature<NoneFeatureConfiguration> {
	public static final MCY1600SpawnFeature FEATURE = (MCY1600SpawnFeature) new MCY1600SpawnFeature().setRegistryName("time:mcy_1600_spawn");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public MCY1600SpawnFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "new1600crystaldungeon"));
		if (template == null)
			return false;
