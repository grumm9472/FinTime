...
public class Mushroom1Feature extends Feature<NoneFeatureConfiguration> {
	public static final Mushroom1Feature FEATURE = (Mushroom1Feature) new Mushroom1Feature().setRegistryName("time:mushroom_1");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Mushroom1Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "end3_13x25_mushroom1"));
		if (template == null)
			return false;
