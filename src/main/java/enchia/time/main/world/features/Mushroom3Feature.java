...
public class Mushroom3Feature extends Feature<NoneFeatureConfiguration> {
	public static final Mushroom3Feature FEATURE = (Mushroom3Feature) new Mushroom3Feature().setRegistryName("time:mushroom_3");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Mushroom3Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "end3_13x25_mushroom2"));
		if (template == null)
			return false;
