...
public class Mushroom4Feature extends Feature<NoneFeatureConfiguration> {
	public static final Mushroom4Feature FEATURE = (Mushroom4Feature) new Mushroom4Feature().setRegistryName("time:mushroom_4");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Mushroom4Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "end3_5x9_mushroomtiny"));
		if (template == null)
			return false;
