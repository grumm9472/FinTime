...
public class Mushroom5Feature extends Feature<NoneFeatureConfiguration> {
	public static final Mushroom5Feature FEATURE = (Mushroom5Feature) new Mushroom5Feature().setRegistryName("time:mushroom_5");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Mushroom5Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (template == null)
			template = context.level().getLevel().getStructureManager()
					.getOrCreate(new ResourceLocation("time", "mushroomislands9x15_giantmushroom"));
		if (template == null)
			return false;
