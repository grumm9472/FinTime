...
public class Mushroom6Feature extends Feature<NoneFeatureConfiguration> {
	public static final Mushroom6Feature FEATURE = (Mushroom6Feature) new Mushroom6Feature().setRegistryName("time:mushroom_6");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Mushroom6Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "plains1x2_mushroom"));
		if (template == null)
			return false;
