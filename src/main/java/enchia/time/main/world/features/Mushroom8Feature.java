...
public class Mushroom8Feature extends Feature<NoneFeatureConfiguration> {
	public static final Mushroom8Feature FEATURE = (Mushroom8Feature) new Mushroom8Feature().setRegistryName("time:mushroom_8");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Mushroom8Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "plains5x7_mushroom"));
		if (template == null)
			return false;
