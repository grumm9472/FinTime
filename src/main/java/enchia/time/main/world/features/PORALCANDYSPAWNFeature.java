...
public class PORALCANDYSPAWNFeature extends Feature<NoneFeatureConfiguration> {
	public static final PORALCANDYSPAWNFeature FEATURE = (PORALCANDYSPAWNFeature) new PORALCANDYSPAWNFeature()
			.setRegistryName("time:poralcandyspawn");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public PORALCANDYSPAWNFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "candyworldportal"));
		if (template == null)
			return false;
