...
public class PhantomTreeNormal3Feature extends Feature<NoneFeatureConfiguration> {
	public static final PhantomTreeNormal3Feature FEATURE = (PhantomTreeNormal3Feature) new PhantomTreeNormal3Feature()
			.setRegistryName("time:phantom_tree_normal_3");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private final List<Block> base_blocks = List.of(TimeModBlocks.DREAM_GRASS);
	private StructureTemplate template = null;

	public PhantomTreeNormal3Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "phantomtreenormal3"));
		if (template == null)
			return false;
