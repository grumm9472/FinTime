...
public class RiserislandFeature extends Feature<NoneFeatureConfiguration> {
	public static final RiserislandFeature FEATURE = (RiserislandFeature) new RiserislandFeature().setRegistryName("time:riserisland");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public RiserislandFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "riseraltar"));
		if (template == null)
			return false;
