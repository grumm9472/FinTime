...
public class Rune5Feature extends Feature<NoneFeatureConfiguration> {
	public static final Rune5Feature FEATURE = (Rune5Feature) new Rune5Feature().setRegistryName("time:rune_5");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = Set.of(new ResourceLocation("time:boimerune"));
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "ruine5spawner"));
		if (template == null)
			return false;
