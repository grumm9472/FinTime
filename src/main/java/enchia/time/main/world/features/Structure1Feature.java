...
public class Structure1Feature extends Feature<NoneFeatureConfiguration> {
	public static final Structure1Feature FEATURE = (Structure1Feature) new Structure1Feature().setRegistryName("time:structure_1");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Structure1Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "102400_house_1"));
		if (template == null)
			return false;
