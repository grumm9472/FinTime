...
public class T2Feature extends Feature<NoneFeatureConfiguration> {
	public static final T2Feature FEATURE = (T2Feature) new T2Feature().setRegistryName("time:t_2");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = Set.of(new ResourceLocation("time:fores_tm_agic"));
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "magic2"));
		if (template == null)
			return false;
