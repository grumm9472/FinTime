...
public class Underg1Feature extends Feature<NoneFeatureConfiguration> {
	public static final Underg1Feature FEATURE = (Underg1Feature) new Underg1Feature().setRegistryName("time:underg_1");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Underg1Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "102400_underground_1"));
		if (template == null)
			return false;
