...
public class Underg2Feature extends Feature<NoneFeatureConfiguration> {
	public static final Underg2Feature FEATURE = (Underg2Feature) new Underg2Feature().setRegistryName("time:underg_2");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public Underg2Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("time", "102400_underground_2"));
		if (template == null)
			return false;
