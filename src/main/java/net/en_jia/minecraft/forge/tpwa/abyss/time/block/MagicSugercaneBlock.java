...
public class MagicSugercaneBlock extends TimeModElements.ModElement {
	@ObjectHolder("time:magic_sugercane")
	public static final Block block = null;

	public MagicSugercaneBlock(TimeModElements instance) {
		super(instance, 570);
	...
	private static class FeatureRegisterHandler {
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, BlockClusterFeatureConfig config) {
					...
					for (int j = 0; j < 6; ++j) {
						...
						if (world.isAirBlock(blockpos)) {
							...
							int k = 1 + random.nextInt(random.nextInt(8) + 1);
							k = Math.min(8, k);
							for (int l = 0; l < k; ++l) {
			...
			event.getRegistry().register(feature.setRegistryName("magic_sugercane"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("time:magic_sugercane"), configuredFeature);
		}
	}
	...
	public static class BlockCustomFlower extends SugarCaneBlock {
		public BlockCustomFlower() {
			super(Block.Properties.create(Material.PLANTS).tickRandomly().doesNotBlockMovement().sound(SoundType.GROUND).hardnessAndResistance(0f, 0f)
					.setLightLevel(s -> 15));
			setRegistryName("magic_sugercane");
		}
