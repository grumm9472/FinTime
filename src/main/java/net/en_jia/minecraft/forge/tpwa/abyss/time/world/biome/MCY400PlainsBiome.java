...
public class MCY400PlainsBiome extends TimeModElements.ModElement {
	public static Biome biome;

	public MCY400PlainsBiome(TimeModElements instance) {
		super(instance, 987);
			...
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-5916161).setWaterColor(-1).setWaterFogColor(-1)
						...
						.setMusic(new BackgroundMusicSelector(
								(net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("time:mcy400b")), 12000,
								24000, true))
						.build();
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.SNOW).category(Biome.Category.NONE).depth(0.1f).scale(0.2f).temperature(0.1f)
						.downfall(1f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("time:mcy_400_plains"));
			}
		}
	}
	...
	private static class CustomLeaveVineTreeDecorator extends LeaveVineTreeDecorator {
		...
		static {
			...
			tdt.setRegistryName("mcy_400_plains_lvtd");
	...
	private static class CustomTrunkVineTreeDecorator extends TrunkVineTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("mcy_400_plains_tvtd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
	...
	private static class CustomCocoaTreeDecorator extends CocoaTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("mcy_400_plains_ctd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
