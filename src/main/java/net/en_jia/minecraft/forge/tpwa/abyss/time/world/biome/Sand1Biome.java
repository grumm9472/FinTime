...
public class Sand1Biome extends TimeModElements.ModElement {
	public static Biome biome;

	public Sand1Biome(TimeModElements instance) {
		super(instance, 1890);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(-3.3000000000000003f).scale(1.1f)
						.temperature(0.5f).downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("time:sand_1"));
			}
		}
	}
