...
import net.en_jia.minecraft.forge.tpwa.abyss.time.procedures.NetherancientPlayerEntersDimensionProcedure;
import net.en_jia.minecraft.forge.tpwa.abyss.time.item.NetherancientItem;
import net.en_jia.minecraft.forge.tpwa.abyss.time.block.MCY100ObsidianBlock;
import net.en_jia.minecraft.forge.tpwa.abyss.time.block.LivinghellstoneBlock;
...
public class NetherancientDimension extends TimeModElements.ModElement {
	@ObjectHolder("time:netherancient_portal")
	public static final CustomPortalBlock portal = null;

	public NetherancientDimension(TimeModElements instance) {
		super(instance, 11);
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(LivinghellstoneBlock.block);
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("time:nether_mcy_100")).getGenerationSettings().getSurfaceBuilder()
	...
	public void clientLoad(FMLClientSetupEvent event) {
		DimensionRenderInfo customEffect = new DimensionRenderInfo(Float.NaN, true, DimensionRenderInfo.FogType.NONE, false, false) {
			...
			public Vector3d func_230494_a_(Vector3d color, float sunHeight) {
				return new Vector3d(0.741176470588, 0.286274509804, 0.286274509804);
			}
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("time:netherancient"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("netherancient_portal", Vector3i::compareTo, 300);
	...
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("netherancient_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())),
					0, 1).setRegistryName("netherancient_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new NetherancientItem().setRegistryName("netherancient"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("netherancient_portal");
		}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("time:netherancient"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos, RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("time:netherancient")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == MCY100ObsidianBlock.block;
		};
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? MCY100ObsidianBlock.block.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, MCY100ObsidianBlock.block.getDefaultState(), 3);
					}
				}
			}
	...
	public void onPlayerChangedDimensionEvent(PlayerEvent.PlayerChangedDimensionEvent event) {
		...
		if (event.getTo() == RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("time:netherancient"))) {

			NetherancientPlayerEntersDimensionProcedure.executeProcedure(Collections.EMPTY_MAP);
		}
	}
}
